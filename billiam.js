...
//main program
var element = document.querySelectorAll("h1" && "h2" && "h3" && "p"); //get all elements with text in them
...
function FindandReplace(input) {
    ...
    for (let word of str) { //loop through each word
        ...
        if (isNaN(word)) { //if word is not a number
            ...
            } else if (["ch","kn","ph","qu","sh","st","th","wh"].includes(word[0].substring(0,2).toLowerCase())) { //else if first 2 letters are a digraph or other 2 letter combo sound
